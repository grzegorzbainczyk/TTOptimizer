...
public class ClassesController : ControllerBase
{
    ...
    public async Task<IActionResult> GenerateClasses(
        [FromQuery] int organizationId,
        [FromBody] GenerateClassesRequest request)
    {
        if (organizationId <= 0)
        {
            return BadRequest(new
        ...
        var maxGrade =
            GetMaxGrade(schoolUnit.SchoolType);
            ...
            item.Count > 26))
        {
            ...
            {
                message =
                    $"Grade must be between 1 and {maxGrade}, " +
                    "and class count between 0 and 26."
            });
        }

        var requested = new List<(string Name, int Grade)>();
        ...
        var toCreate = requested
            .Where(item =>
                !existingSet.Contains(item.Name))
            .ToList();

        var skippedExistingCount =
            requested.Count - toCreate.Count;
        ...
        try
        {
            foreach (var item in toCreate)
            {
                ...
                _db.StudentGroups.Add(
                    new StudentGroup
                    {
                        OrganizationId = organizationId,
                        Name = item.Name,
                        Type =
                            StudentGroupType.WholeClass,
                        ClassGroup = classGroup
                    }
                );
            }
        ...
        catch (DbUpdateException)
        {
            ...
            {
                message =
                    "One or more generated classes already exist."
            });
        }
        ...
        return Ok(new
        {
            ...
            createdNames =
                toCreate.Select(item => item.Name)
        });
    }
