...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const generateClassesButton =
        document.getElementById("generateClassesButton");
    ...
    generateClassesButton?.addEventListener(
        "click",
        openClassGenerator
    );

    document.getElementById("closeClassGeneratorButton")
        ?.addEventListener("click", closeClassGenerator);

    document.getElementById("cancelClassGeneratorButton")
        ?.addEventListener("click", closeClassGenerator);

    document.getElementById("confirmClassGeneratorButton")
        ?.addEventListener("click", generateClasses);

    document.getElementById("classGeneratorOverlay")
        ?.addEventListener("click", event => {
            if (event.target?.id === "classGeneratorOverlay") {
                closeClassGenerator();
            }
        });
...
function openClassGenerator() {
    ...
    const maxGrade =
        getMaxGradeForSchoolUnit(schoolUnit.id);

    const rows =
        document.getElementById("classGeneratorRows");

    const info =
        document.getElementById("classGeneratorSchoolInfo");
    ...
    if (info) {
        info.textContent =
            `${schoolUnit.name} • roczniki 1–${maxGrade}`;
    }

    for (let grade = 1; grade <= maxGrade; grade++) {
        const row = document.createElement("label");
        row.className = "class-generator-row";

        const label = document.createElement("span");
        label.textContent = `Rocznik ${grade}`;
        ...
        input.addEventListener(
            "input",
            renderClassGeneratorPreview
        );

        const suffix = document.createElement("span");
        suffix.className = "class-generator-row-suffix";
        suffix.textContent = "liczba klas";

        row.append(label, input, suffix);
        rows.appendChild(row);
    }

    clearClassGeneratorMessage();
    renderClassGeneratorPreview();
    overlay.hidden = false;

    const firstInput =
        rows.querySelector("input");

    firstInput?.focus();
    firstInput?.select();
}

function closeClassGenerator() {
    const overlay =
        document.getElementById("classGeneratorOverlay");
...
function getGeneratorSchoolUnitId() {
    const importSchoolUnitId =
        document.getElementById(
            "classImportSchoolUnitId"
        )?.value;

    if (importSchoolUnitId) {
        return Number(importSchoolUnitId);
    }

    return Number(
        availableSchoolUnits[0]?.id ?? 0
    );
}

function getSuggestedClassCount(
    schoolUnitId,
    grade
) {
    const count =
        availableClasses.filter(item =>
            Number(item.schoolUnitId) ===
                Number(schoolUnitId) &&
...
function getClassGeneratorCounts() {
    return Array.from(
        document.querySelectorAll(
            "#classGeneratorRows input[data-grade]"
        )
    ).map(input => ({
...
function buildGeneratedClassNames(counts) {
    const result = [];
...
function renderClassGeneratorPreview() {
    const preview =
        document.getElementById(
            "classGeneratorPreview"
        );

    const confirmButton =
        document.getElementById(
            "confirmClassGeneratorButton"
        );
    ...
    const generated =
        buildGeneratedClassNames(
            getClassGeneratorCounts()
        );
    ...
    if (confirmButton) {
        ...
        confirmButton.textContent =
            `Utwórz ${generated.length} ` +
            getPolishClassWord(generated.length);
    }
}

function getPolishClassWord(count) {
    if (count === 1) {
        return "klasę";
    }
...
async function generateClasses() {
    ...
    const requested =
        buildGeneratedClassNames(counts);

    if (requested.length === 0) {
        showClassGeneratorMessage(
            "Wybierz co najmniej jedną klasę.",
            true
        );
        return;
    }
    ...
    try {
        const response = await fetch(
            `/api/classes/generate?organizationId=${encodeURIComponent(
                organizationId
            )}`,
            {
                method: "POST",
        ...
        if (!response.ok) {
            throw new Error(
                getApiErrorMessage(
                    data,
                    `Nie udało się wygenerować klas. Status: ${response.status}`
                )
            );
        }
        ...
        const message =
            skipped > 0
                ? `Utworzono ${created} klas. Pominięto ${skipped}, ponieważ już istniały.`
                : `Utworzono ${created} klas.`;
        ...
        showClassGeneratorMessage(
            error instanceof Error
                ? error.message
                : "Nie udało się wygenerować klas.",
            true
        );
...
function showClassGeneratorMessage(
    message,
    isError
) {
    const element =
        document.getElementById(
            "classGeneratorMessage"
        );
...
function clearClassGeneratorMessage() {
    const element =
        document.getElementById(
            "classGeneratorMessage"
        );
