...
async function refresh() {
    ...
    if (!classResponse.ok || !groupResponse.ok) {
        showMessage("Could not load student groups.", true); return;
    }
...
function renderMembers() {
    ...
    if (selectable.length === 0) { host.textContent = "Create subgroups or individual groups first."; return; }
...
function renderTree() {
    ...
    for (const classGroup of classes) {
        ...
        section.append(createRow(whole?.name ?? classGroup.name, "Whole class", null));
        ...
        for (const division of divisions) {
            ...
            for (const group of division.groups) block.append(createRow(group.name, "Subgroup", group));
            section.append(block);
        }
        const individuals = overview.groups.filter(g => g.classGroupId === classGroup.id && (g.type === 3 || g.type === "Individual"));
        for (const group of individuals) section.append(createRow(group.name, "Individual", group));
        host.append(section);
    }
    ...
    if (combined.length) {
        ...
        const title = document.createElement("h3"); title.textContent = "Combined groups"; section.append(title);
        for (const group of combined) section.append(createRow(`${group.name} (${(group.memberGroupNames ?? []).join(", ")})`, "Combined", group));
        host.append(section);
    }
}
...
function smallDeleteButton(handler) { const b=document.createElement("button"); b.type="button"; b.className="small-button teacher-delete-button"; b.textContent="Delete"; b.addEventListener("click",handler); return b; }
...
async function post(path, body) {
    ...
    const data=await read(response); if(!response.ok){showMessage(data?.message??"Could not save student group.",true);return;} showMessage("Student groups updated.",false); await refresh();
}
async function deleteGroup(group) { if(!confirm(`Delete student group ${group.name}?`)) return; await del(`${group.id}`); }
async function deleteDivision(division) { if(!confirm(`Delete division ${division.name}?`)) return; await del(`divisions/${division.id}`); }
async function del(path) { const organizationId=window.appContext.requireOrganizationId(); const response=await fetch(`/api/student-groups/${path}?organizationId=${encodeURIComponent(organizationId)}`,{method:"DELETE"}); const data=await read(response); if(!response.ok){showMessage(data?.message??"Could not delete student group.",true);return;} showMessage("Student groups updated.",false); await refresh(); }
