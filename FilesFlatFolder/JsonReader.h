...
private:
    ...
    static LessonPriority parseLessonPriority(int value)
    {
        switch (value)
        {
        case 0:
            return LessonPriority::Low;
        case 1:
            return LessonPriority::Normal;
        case 2:
            return LessonPriority::High;
        default:
            throw std::runtime_error(
                "Unknown lesson priority value: " +
                std::to_string(value));
        }
    }
        ...
        TimetableProblem& problem)
    {
        ...
        for (const auto& item : root["lessonRequirements"])
        {
            ...
            requirement.isAdditional = item.value("isAdditional", false);
            requirement.priority = parseLessonPriority(
                item.value("priority", 1));
